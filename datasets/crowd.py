...
def center_crop(im_h, im_w, crop_h, crop_w):
    res_h = im_h - crop_h
    res_w = im_w - crop_w
    i = res_h//2
    j = res_w//2
...
class Crowd(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.method == 'train':
            ...
            keypoints = np.load(gt_path)
            return self.test_transform(RGB, T, keypoints, gt_path)
    ...
    def test_transform(self, RGB, T, keypoints, gt_path):
        ht, wd, _ = RGB.shape
        st_size = 1.0 * min(wd, ht)
        assert st_size >= self.c_size
        assert len(keypoints) > 0
        i, j, h, w = center_crop(ht, wd, self.c_size, self.c_size)
        RGB = RGB[i:i+h, j:j+w, :]
        T = T[i:i+h, j:j+w, :]
        keypoints = keypoints - [j, i]
        idx_mask = (keypoints[:, 0] >= 0) * (keypoints[:, 0] <= w) * \
