...
class RegTrainer(Trainer):
    ...
    def train_eopch(self):
        ...
        for step, (inputs, points, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                outputs, out_fea, x_anchor = self.model(inputs)
                # print ('outputs shape',outputs.shape)
                prob_list = self.post_prob(points, st_sizes)
                loss1 = self.criterion(prob_list, outputs)
                loss2 = self.mse(out_fea, x_anchor)
                loss = loss1 + loss2
    ...
    def val_epoch(self):
        ...
        logging.info('Epoch {} Val{}, '
                     'GAME0 {game0:.2f} GAME1 {game1:.2f} GAME2 {game2:.2f} GAME3 {game3:.2f} RMSE {mse:.2f} mae {mae:.4f} Re {relative:.4f}, '
                     .format(self.epoch, N, game0=game[0], game1=game[1], game2=game[2], game3=game[3], mse=mse[0], mae=MAE[0] , relative=total_relative_error
                             )
                     )
        ...
        if game[0] < self.best_game0 or game[3] < self.best_game3:
            ...
            if args.save_all_best:
                torch.save(model_state_dic, os.path.join(self.save_dir, 'best_model_e{}.pth'.format(self.epoch)))   ### from 03 22 2023 
                self.best_count += 1
            else:
    ...
    def test_epoch(self):
        ...
        logging.info('Epoch {} Test{}, '
                     'GAME0 {game0:.2f} GAME1 {game1:.2f} GAME2 {game2:.2f} GAME3 {game3:.2f} RMSE {mse:.2f} mae {mae:.4f} Re {relative:.4f}, '
                     .format(self.epoch, N, game0=game[0], game1=game[1], game2=game[2], game3=game[3], mse=mse[0],  mae=MAE[0] ,
